 ...
 public function index()
  {
     ...
     $data=array(
          'active_dataobat'=>'active',
          'ambil_info'=>$this->m_dataobat->TampilData('dataobat'),

          );
        $this->load->helper('url');
        $this->load->library('pagination');//1
        $this->load->database();//memanggil pengaturan database dan mengaktifkannya
        $this->load->model('m_dataobat');//memanggil model m_user
        ...
        $config['base_url'] = site_url('dataobat');
      ...
      $config['last_tagl_close'] = "</li>";
        ...
        $data['dataobat'] = $result['data'];
       
        $this->load->view('admin/element/header');
        $this->load->view('admin/element/sidebar',$data);
        $this->load->view('admin/v_tambahdataobat',$data);
...
{
       $data=array(
          'active_dataobatt'=>'active',
          // 'id'=>$this->m_dataobat->getKodeInfo(),

      );
   
    $this->load->view('admin/element/header');
      $this->load->view('admin/element/sidebar',$data);
      $this->load->view('admin/v_formtambahdataobat');
}
...
public function prosestambah(){
    ...
    $this->m_dataobat->insertData('dataobat',$data); //akses model untuk menyimpan ke database

      if($data >= 1) {
              $this->session->set_flashdata("pesan", "<div class=\"col-md-12\"><div class=\"alert alert-success\">Data Berhasil Ditambahkan </div></div>");
              redirect('dataobat'); //jika berhasil maka akan ditampilkan view vupload
          }else{
              ...
              redirect('dataobat'); //jika gagal maka akan ditampilkan form upload
          }
