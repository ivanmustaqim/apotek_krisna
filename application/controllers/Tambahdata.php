...
class Tambahdata extends CI_Controller {
 ...
 //    $this->load->view('admin/element/header');
  ...
  public function index()
  {
    ...
    $data['ambil_info']=$this->modelapp->TampilData($tgl)->result();
    $this->load->view('admin/element/header');
      $this->load->view('admin/element/sidebar',$data);
      $this->load->view('admin/v_tambahdata',$data);
  ...
  public function adddata()
  {
    ...
    //   );
      ...
      $category = $this->modelapp->get_jenisobat();
    $category_options = array();

     $category_options['']='--BELUM DIPILIH--';

    if($category->num_rows() > 0){
      foreach ($category->result_array() as $row) {
        $category_options[$row['id_jenis_obat']] = strtoupper($row['jenis_obat']);
      }
    }
    ...
    $this->load->view('admin/element/header');
    $this->load->view('admin/element/sidebar',$data);
    $this->load->view('admin/v_formtambahdata',$data);
  }
  ...
  public function prosestambah(){
   ...
   if($data >= 1) {
     $this->session->set_flashdata("pesan", "<div class=\"col-md-12\"><div class=\"alert alert-success\">Data Berhasil Ditambahkan </div></div>");
							redirect('tambahdata'); //jika berhasil maka akan ditampilkan view vupload
           }else{
             ...
             $this->session->set_flashdata("pesan", "<div class=\"col-md-12\"><div class=\"alert alert-danger\">Gagal Tambah coba lagi !</div></div>");
							redirect('tambahdata'); //jika gagal maka akan ditampilkan form upload
           }
