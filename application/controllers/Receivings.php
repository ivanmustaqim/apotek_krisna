...
class Receivings extends CI_Controller {
  ...
  //  );
		...
		$this->load->view('receivings/v_masuk',$data);
  ...
  public function cariobat(){
	...
	public function simpan(){
		$cart = $this->cart->contents();
			...
			$id_user = $this->session->userdata('id_user');
			$tgl_masuk = date('Y-m-d',strtotime($this->input->post('tgl_receivings'))	);
	...
	public function detil_masuk(){
		$data['receivings'] = $this->modelapp2->ambil_data()->result();
		$this->load->view('receivings/detil_masuk', $data);
	}

	public function detil_obat($sale_id){
		$data['obat'] = $this->modelapp2->ambil_obat($sale_id)->result();
		$this->load->view('receivings/detil_obat', $data);
	}

	public function hapus_detil_masuk($receivings_id){
		if($this->modelapp2->hapus($receivings_id) > 0){
			$pesan = '<div class="alert alert-info">DATA BERHASIL DIHAPUS</div>';
		}else{
			$pesan = '<div class="alert alert-danger">DATA GAGAL DIHAPUS</div>';
		}
		$this->session->set_flashdata('pesan', $pesan);
		redirect('receivings/detil_masuk');
	}		
