...
class M_masuk extends CI_Model{
    ...
    function ambil_data(){
        $this->db->where('masuk.deleted', 0);
        $this->db->join('user', 'user.id_user = masuk.id_user');
        ...
        $this->db->join('obat', 'obat.id_obat = masuk_items.id_obat ');
        return $this->db->get('masuk_items');
    }

        function hapus($sale_id){
        $this->db->where('sale_id', $sale_id);
        $this->db->update('masuk', array('deleted'=>1));
